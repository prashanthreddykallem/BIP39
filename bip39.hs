-- |Haskell program to Convert a private key (Ed25519) to a mnemonic based on BIP39 Standard 
...
finalEntropy::BS.ByteString->BS.ByteString
finalEntropy byStr = BS.append byStr $ BS.take cs hash
    where cs = (BS.length byStr) `div` 32
          hash = SHA256.hash byStr  
mnemonicConvert::BS.ByteString->String
mnemonicConvert byStr=unwords $ map(wl V.!) index
    where index = bsToIndex byStr
bsToIndex :: BS.ByteString->[Int]
bsToIndex byStr = reverse .go q $ bsToInteger byStr
    where q = ((BS.length byStr)*8) `div ` 11
          go 0 _ = []
...
main=do
    (pk,sk) <- createKeypair
    let ent = unSecretKey sk 
    print ent    
    let ms = mnemonicConvert $ finalEntropy ent     
    print ms 
